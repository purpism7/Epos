    ...
    public class Adaptive : BaseStrategy
    {
        ...
        public override async UniTask RegroupToLeaderAsync(CancellationToken cancellationToken)
        {
            ...
            try 
            {
                ...
                {
                    TraceTo(combatant1, DirectionType.Back, 7f, true);
                ...
                // --- 두 번째 유닛 이동 지시 ---
                var combatant2 = _strategyDataProvider?.AllyICombatantList?.Find(combatant => combatant.IActor.Id == 10001);
                if (combatant2?.IActor?.IActCtr != null)
                {
                    TraceTo(combatant2, DirectionType.Forward, 7f, true);
